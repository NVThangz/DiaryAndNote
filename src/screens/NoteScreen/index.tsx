...
const NoteScreen: FC<IProps> = ({ navigation }, props) => {
  ...
  useEffect(() => {
    ...
    else navigation.setOptions({headerShown: true, headerRight: () => (
      <View style={{flexDirection: "row"}}>
        {/* <TouchableOpacity onPress={enableViewMode}>
  ...
  const handleOnSave = () => {
    ...
    if(!title) {
      Alert.alert("Warning",'Please write your title')
      return;
    }
    if(note) {
      const newNote : noteState= {...note, desc: desc, title: title, date: date.toString(), font: font, position: position, medias: medias, stickers: stickers}
      if(JSON.stringify(note) === JSON.stringify(newNote) ) {
        Alert.alert("Warning",'Note doesn\'t change')
        return;
       }
      dispatch(updateNote(newNote));
      navigation.goBack();
      Alert.alert("Success",'Note changed')
      return;
    }
    const newNote : noteState = { date: date.toString() ,desc: desc, title: title, id: uuidv4(), font: font, position: position, medias: medias, stickers: stickers}
