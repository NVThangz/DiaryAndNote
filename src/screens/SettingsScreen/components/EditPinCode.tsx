...
const  EditPinCode = ({ navigation, setVisible}) => {
    ...
    const check2ndPinCode = async () => {
        console.log(passCode, tempPassCode)
        if (JSON.stringify(passCode) === JSON.stringify(tempPassCode)) {
          alert("Create PIN Code success");
          let pinCodeParsed : pinCode = await getPINCode()
          pinCodeParsed.code = passCode
